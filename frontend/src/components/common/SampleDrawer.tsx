import { Alert, Descriptions, Drawer, Tabs, Typography } from 'antd'
...
import { SpecimenIsolationTag } from './IsolationBadge'
...
export function SampleDrawer({ specimen, open, onClose }: { specimen: Specimen | null; open: boolean; onClose: () => void }) {
  return (
    <Drawer width={720} title={specimen ? `样本 ${specimen.accessionNo}` : '样本详情'} open={open} onClose={onClose}>
      {specimen && <Tabs items={[
        { key: 'profile', label: '基本信息', children: <div className="drawer-stack">{specimen.isolated && <Alert type="error" showIcon message={<SpecimenIsolationTag specimen={specimen} />} description="禁止交接与批准放行，可暂缓或拒绝复核" />}<Descriptions bordered size="small" column={2}><Descriptions.Item label="保管状态"><CustodyBadge state={specimen.state} /></Descriptions.Item><Descriptions.Item label="冷链状态">{specimen.isolated ? <SpecimenIsolationTag specimen={specimen} /> : '正常'}</Descriptions.Item><Descriptions.Item label="样本类型">{specimen.sampleType}</Descriptions.Item><Descriptions.Item label="受试者编码">{specimen.subjectCode}</Descriptions.Item><Descriptions.Item label="研究协议">{specimen.protocolCode}</Descriptions.Item><Descriptions.Item label="当前保管人">{specimen.currentCustodian}</Descriptions.Item><Descriptions.Item label="接收时间">{formatDateTime(specimen.receivedAt)}</Descriptions.Item><Descriptions.Item label="体积/分装">{specimen.volumeMl} mL / {specimen.aliquotCount} 份</Descriptions.Item><Descriptions.Item label="有效期">{formatDateTime(specimen.expiresAt)}</Descriptions.Item><Descriptions.Item label="冻存容器">{specimen.storageContainer ? `${specimen.storageContainer.code} · ${specimen.storageContainer.name}` : '待分配'}</Descriptions.Item><Descriptions.Item label="位置">{specimen.position || '-'}</Descriptions.Item><Descriptions.Item label="温区">{specimen.storageContainer ? <StatusBadge value={specimen.storageContainer.temperatureZone} /> : '-'}</Descriptions.Item><Descriptions.Item label="备注" span={2}>{specimen.notes || '-'}</Descriptions.Item></Descriptions></div> },
        { key: 'isolation', label: `隔离历史 (${specimen.isolationEvents?.length || 0})`, children: specimen.isolationEvents?.length ? specimen.isolationEvents.map((event) => <div className="review-record" key={event.id}><div>{event.active ? <SpecimenIsolationTag specimen={specimen} /> : <StatusBadge value="anomaly_resolved" />} <Typography.Text strong>{event.temperatureAnomaly?.anomalyNo || `异常 #${event.temperatureAnomalyId}`}</Typography.Text></div><Typography.Text>{event.isolatedByName} 隔离 · {formatDateTime(event.isolatedAt)}</Typography.Text>{event.releasedAt && <Typography.Paragraph type="secondary">{event.releasedByName} 解除 · {formatDateTime(event.releasedAt)} · 依据：{event.releaseBasis}</Typography.Paragraph>}</div>) : <Typography.Text type="secondary">暂无隔离记录</Typography.Text> },
