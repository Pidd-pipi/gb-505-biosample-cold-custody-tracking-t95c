  ...
  return (
    <div className="page-stack">
      ...
      <Modal title="发起样本交接" width={700} open={createOpen} confirmLoading={saving} onOk={() => void create()} onCancel={() => setCreateOpen(false)} okText="创建交接单" cancelText="取消">
        ...
        }}><Row gutter={16}><Col span={12}><Form.Item name="transferNo" label="交接单号" rules={[{ required: true, min: 3 }]}><Input placeholder="TR-20260822-001" /></Form.Item></Col><Col span={12}><Form.Item name="specimenId" label="样本" rules={[{ required: true }]}><Select showSearch optionFilterProp="label" options={specimens.map((item) => ({ value: item.id, disabled: Boolean(item.quarantineAnomalyId), label: `${item.accessionNo} · ${item.sampleType}${item.quarantineAnomalyId ? '（冷链隔离，禁止交接）' : ''}` }))} /></Form.Item></Col></Row><Row gutter={16}><Col span={12}><Form.Item name="fromCustodian" label="移交人" rules={[{ required: true }]}><Input /></Form.Item></Col><Col span={12}><Form.Item name="toCustodian" label="接收人" rules={[{ required: true }]}><Input /></Form.Item></Col></Row><Row gutter={16}><Col span={12}><Form.Item name="fromLocation" label="移交前位置" rules={[{ required: true }]}><Input /></Form.Item></Col><Col span={12}><Form.Item name="toLocation" label="目标位置描述" rules={[{ required: true }]}><Input /></Form.Item></Col></Row><Form.Item name="temperatureC" label="交接温度 (°C)"><InputNumber min={-200} max={30} precision={1} style={{ width: '100%' }} /></Form.Item><Form.Item name="reason" label="交接用途/备注"><Input.TextArea rows={3} maxLength={800} showCount /></Form.Item></Form>
      </Modal>
