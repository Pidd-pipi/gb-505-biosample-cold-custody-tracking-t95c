import { ExclamationCircleOutlined, PlusOutlined, SearchOutlined, WarningOutlined } from '@ant-design/icons'
import { Button, Col, Form, Input, InputNumber, Modal, Progress, Row, Select, Space, Statistic, Tooltip, Typography, message } from 'antd'
...
export function StoragePage() {
  ...
  const [reportContainer, setReportContainer] = useState<number | undefined>()
  ...
  const openAnomalyCount = data.items.filter((item) => item.anomalyStatus === 'open').length
  const isolatedTotal = data.items.reduce((sum, item) => sum + (item.isolatedSpecimenCount || 0), 0)
  const columns: ColumnsType<StorageContainer> = [
    ...
    {
      title: '冷链异常', width: 170, render: (_, row) => row.anomalyStatus === 'open'
        ? <Tooltip title="点击查看未结异常与涉及样本，温度恢复后须由非建单人解除"><Button type="link" danger size="small" className="table-link" icon={<ExclamationCircleOutlined />} onClick={() => setAnomalyId(row.openAnomalyId ?? null)}>隔离中 · {row.isolatedSpecimenCount || 0} 份</Button></Tooltip>
    ...
    { title: '最近更新', dataIndex: 'updatedAt', render: formatDateTime },
    {
      title: '操作', fixed: 'right', render: (_, row) => (
        <Space>
          {can('anomaly:manage') && (
            row.anomalyStatus === 'open'
              ? <Button size="small" danger icon={<ExclamationCircleOutlined />} onClick={() => setAnomalyId(row.openAnomalyId ?? null)}>未结异常</Button>
              : <Tooltip title="巡检发现读数超限时提交，将隔离容器内已冻存样本"><Button size="small" icon={<WarningOutlined />} onClick={() => { setReportContainer(row.id); setReportOpen(true) }}>超限上报</Button></Tooltip>
          )}
        </Space>
      ),
    },
  ...
  return (
    <div className="page-stack">
      <header className="page-header"><div><Typography.Title level={2}>冻存位置</Typography.Title><Typography.Text type="secondary">管理冷冻柜、液氮罐及其温区、容量与物理位置；超限读数触发样本隔离闭环</Typography.Text></div><Space>{can('anomaly:manage') && <Button danger icon={<WarningOutlined />} onClick={() => { setReportContainer(undefined); setReportOpen(true) }}>巡检超限上报</Button>}{can('storage:write') && <Button type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>新增容器</Button>}</Space></header>
      <Row gutter={16}><Col xs={24} sm={8}><div className="metric"><Statistic title="容器总数" value={data.total} /></div></Col><Col xs={24} sm={8}><div className="metric"><Statistic title="未结冷链异常" value={openAnomalyCount} valueStyle={{ color: openAnomalyCount ? '#cf1322' : undefined }} /></div></Col><Col xs={24} sm={8}><div className="metric"><Statistic title="隔离样本（本页）" value={isolatedTotal} valueStyle={{ color: isolatedTotal ? '#cf1322' : undefined }} /></div></Col></Row>
      ...
      <ReportAnomalyModal open={reportOpen} containers={data.items} defaultContainerId={reportContainer} onClose={() => setReportOpen(false)} onReported={() => void refresh()} />
